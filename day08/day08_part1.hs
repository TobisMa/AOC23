...
solve inp = show (res)
    where
        ...
        res = countWalk pmap (cycle directions) "AAA"

countWalk :: Map String (String, String) -> String -> String -> Integer
countWalk walk (d:directions) pos
    | pos == "ZZZ" = 0
    | d == 'L' = 1 + countWalk walk directions (fst (walk ! pos))
    | d == 'R' = 1 + countWalk walk directions (snd (walk ! pos))
